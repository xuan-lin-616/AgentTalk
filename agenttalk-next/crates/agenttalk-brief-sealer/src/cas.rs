...
impl CoreCas {
    ...
    pub fn ensure_objects_root(&self) -> Result<(), CasError> {
        fs_guard::ensure_no_reparse_for_path(&self.project_root, &self.cas_root()).map_err(
            |source| CasError::Io {
                path: self.cas_root(),
                source,
            },
        ...
        fs_guard::ensure_no_reparse_for_path(&self.project_root, &self.objects_root()).map_err(
            |source| CasError::Io {
                path: self.objects_root(),
                source,
            },
    ...
    /// Durable, atomic, no-replace publish of exact bytes. Re-publishing the
    /// same bytes is idempotent and does not create a second object.
    pub fn publish(&self, bytes: &[u8]) -> Result<CasObject, CasError> {
        ...
        match fs::symlink_metadata(&destination) {
            Ok(metadata) => {
                if fs_guard::is_reparse_point(&metadata) {
                    return Err(CasError::ReparsePoint { path: destination });
                }
                let existing = fs::read(&destination).map_err(|source| CasError::Io {
        ...
        match fs::hard_link(&temporary, &destination) {
            ...
            {
                let _ = fs::remove_file(&temporary);
                let existing = fs::read(&destination).map_err(|error| CasError::Io {
                    path: destination,
                    source: error,
                })?;
                if existing == bytes {
    ...
    pub fn read(&self, object_ref: &str) -> Result<Vec<u8>, CasError> {
        ...
        let bytes = fs::read(&path).map_err(|source| CasError::Io { path, source })?;
